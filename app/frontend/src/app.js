...
import { viewJaneDashboard, bindJaneDashboard, refreshJaneDashboard } from './views/jane-dashboard.js';
...
async function loadCms() {
  ...
  state.cms.error = lastErr?.message || String(lastErr || 'CMS request failed');
...
function shellSidebar(isAuthed) {
  ...
  return `
    <aside class="sidebar" role="navigation" aria-label="App">
      ...
      <a class="nav-link" href="#/app/jane"${r === '/app/jane' ? ' aria-current="page"' : ''}>Jane + Zoom</a>
      ${adminLink}
      ${
...
function bind() {
  ...
  if (state.route === '/app/jane') bindJaneDashboard(render);
...
async function routeChange() {
  ...
  if (state.user && r === '/app/jane') {
    await refreshJaneDashboard(render);
    return;
  }
  render();
}
